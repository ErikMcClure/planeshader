...
// Constructors
psDirectX10::psDirectX10(const psVeciu& dim, unsigned int antialias, bool vsync, bool fullscreen, HWND hwnd) : psDriver(dim), _device(0), _vsync(vsync), _lasterr(0),
...
void BSS_FASTCALL psDirectX10::Draw(psVertObj* buf, FLAG_TYPE flags, const float(&transform)[4][4])
{
  ...
  {
    _device->IASetIndexBuffer((ID3D10Buffer*)buf->indices, (DXGI_FORMAT)FMTtoDXGI(buf->ifmt), 0);
...
void BSS_FASTCALL psDirectX10::DrawLines(const psLine& line, float Z1, float Z2, unsigned long vertexcolor, const float(&transform)[4][4])
{
...
void psDirectX10::DrawLinesEnd(const float(&transform)[4][4])
{
  PROFILE_FUNC();
...
void BSS_FASTCALL psDirectX10::SetExtent(float znear, float zfar) { _extent.x = znear; _extent.y = zfar; }
void* BSS_FASTCALL psDirectX10::CreateBuffer(size_t bytes, unsigned int usage, const void* initdata)
{
  PROFILE_FUNC();
...
void* BSS_FASTCALL psDirectX10::CreateTexture(psVeciu dim, FORMATS format, unsigned int usage, unsigned char miplevels, const void* initdata, void** additionalview, psTexblock* texblock)
{
  ...
  D3D10_TEXTURE2D_DESC desc = { dim.x, dim.y, miplevels, 1, (DXGI_FORMAT)FMTtoDXGI(format), { 1, 0 }, (D3D10_USAGE)_usagetodxtype(usage), _usagetobind(usage), _usagetocpuflag(usage), _usagetomisc(usage) };
  D3D10_SUBRESOURCE_DATA subdata = { initdata, (((psColor::BitsPerPixel(format)*dim.x) + 7) >> 3), 0 }; // The +7 here is to force the per-line bits to correctly round up the number of bytes.
...
void psDirectX10::_getbackbufferref()
{
  ID3D10Texture2D* backbuffer = 0;
  LOGFAILURE(_swapchain->GetBuffer(0, __uuidof(ID3D10Texture2D), (LPVOID*)&backbuffer));
...
void psDirectX10::_resetscreendim()
{
...
void* BSS_FASTCALL psDirectX10::CompileShader(const char* source, SHADER_VER profile, const char* entrypoint)
{
  ...
  ID3D10Blob* ret = 0;
  ID3D10Blob* err = 0;
  if(FAILED(_lasterr = D3DX10CompileFromMemory(source, strlen(source), 0, 0, 0, entrypoint, shader_profiles[profile], 0, 0, 0, &ret, &err, 0)))
  {
...
void psDirectX10::_setcambuf(ID3D10Buffer* buf, const float(&cam)[4][4], const float(&world)[4][4])
{
