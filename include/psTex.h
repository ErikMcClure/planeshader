...
namespace planeshader {
  ...
  public:
    ...
    static psTex* BSS_FASTCALL Create(const char* file, unsigned int usage = USAGE_SHADER_RESOURCE, FILTERS mipfilter = FILTER_TRIANGLE, unsigned char miplevels = 0, FILTERS loadfilter = FILTER_NONE, bool sRGB = false, psTexblock* texblock = 0, psVeciu dpi = psVeciu(psDriver::BASE_DPI));
    ...
    static psTex* BSS_FASTCALL Create(const void* data, unsigned int datasize, unsigned int usage = USAGE_SHADER_RESOURCE, FILTERS mipfilter = FILTER_TRIANGLE, unsigned char miplevels = 0, FILTERS loadfilter = FILTER_NONE, bool sRGB = false, psTexblock* texblock = 0, psVeciu dpi = psVeciu(psDriver::BASE_DPI));
    static psTex* BSS_FASTCALL Create(const psTex& copy);
    static psTex* BSS_FASTCALL Create(psVeciu dim, FORMATS format, unsigned int usage, unsigned char miplevels = 0, psTexblock* texblock = 0, psVeciu dpi = psVeciu(psDriver::BASE_DPI));
