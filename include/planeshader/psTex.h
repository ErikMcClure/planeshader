...
namespace planeshader {
  ...
  public:
    ...
    psTex(psVeciu dim, FORMATS format, uint32_t usage, uint8_t miplevels=0, psTexblock* texblock=0, psVeciu dpi = psVeciu(BASE_DPI));
    psTex(void* res, void* view, psVeciu dim, FORMATS format, uint32_t usage, uint8_t miplevels, psTexblock* texblock=0, psVeciu dpi = psVeciu(BASE_DPI)); // used to manually set res and view
    ~psTex();
    inline void* GetRes() const { return _res; }
    inline void* GetView() const { return _view; }
    inline psVec GetDim() const { return psVec(_dim)*(psVec((float)BASE_DPI) / psVec(_dpi)); }
    ...
    inline void Clear(psColor32 color) { _driver->Clear(this, color); }
    ...
    static psTex* Create(const char* file, uint32_t usage = USAGE_SHADER_RESOURCE, FILTERS mipfilter = FILTER_TRIANGLE, uint8_t miplevels = 0, FILTERS loadfilter = FILTER_NONE, bool sRGB = false, psTexblock* texblock = 0, psVeciu dpi = psVeciu(BASE_DPI));
    ...
    static psTex* Create(const void* data, uint32_t datasize, uint32_t usage = USAGE_SHADER_RESOURCE, FILTERS mipfilter = FILTER_TRIANGLE, uint8_t miplevels = 0, FILTERS loadfilter = FILTER_NONE, bool sRGB = false, psTexblock* texblock = 0, psVeciu dpi = psVeciu(BASE_DPI));
    static psTex* Create(const psTex& copy);
    static psTex* Create(psVeciu dim, FORMATS format, uint32_t usage, uint8_t miplevels = 0, psTexblock* texblock = 0, psVeciu dpi = psVeciu(BASE_DPI));
