...
namespace planeshader {
  struct BSS_COMPILER_DLLEXPORT cMouseData
  {
    ...
    psVeci relcoord;
    bss_util::cBitField<unsigned char> button; //use the mousebuttons enum for the corresponding array index here to see if that button is pressed (true=pressed, false=not pressed)
    short scrolldelta; //The last change in the scroll wheel
  };

  struct BSS_COMPILER_DLLEXPORT JOY_DEVCAPS
  {
    static const unsigned char NUMAXIS=6; //Max number of axis accounted for
  ...
  public:
    ...
    inline bool GetButton(unsigned short button) const { return (_allbuttons[button>>8]&(1<<(button&0xFF)))!=0; }
    ...
    inline float GetAxis(unsigned short axis) const { return _translatejoyaxis(axis); }
    ...
    inline bool HasJoystick(unsigned char joy) const { return (_alljoysticks&(1<<joy))!=0; }
    ...
    static const unsigned char NUMAXIS=JOY_DEVCAPS::NUMAXIS; //Max number of axis accounted for
  ...
  protected:
    ...
    unsigned short _alljoysticks; //Each bit, if set to 1, indicates the joystick corresponding to the bit position is connected
